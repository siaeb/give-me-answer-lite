...
class GMA_Top_Responders extends WP_Widget {
    /**
     * Constructor
     *
     * @return void
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'gma-widget gma-top-responders', 'description' => __( 'Show the list of top responders.', 'give-me-answer-lite' ) );
        parent::__construct( 'gma-top-responders', __( 'Give Me Answer : Top Responders', 'give-me-answer-lite' ), $widget_ops );
    }
    ...
    function widget( $args, $instance ) {
        ...
        if ( $data ) {
            echo '<div class="gma-widget gma-top-responders gma">';
    ...
    function form( $instance ) {
        $instance = wp_parse_args( $instance, array(
            'title'     => __('Top Responders', 'give-me-answer-lite'),
            'number'    => 6,
        ) );
        ?>
    ...
    private function get_data_from_database($limit = 10) {
        ...
        $query = "
            SELECT u.ID `user_id`, u.user_login, COUNT(p.ID) `qcount` FROM {$wpdb->users} u
                INNER JOIN {$wpdb->posts} p ON p.post_author = u.ID AND p.post_type = 'gma-answer' AND p.post_status = 'publish'
                INNER JOIN {$wpdb->posts} p2 ON p2.ID = p.post_parent AND p2.post_status = 'publish'
