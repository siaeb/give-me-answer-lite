...
class GMA_Notifications {


	public function __construct() {

		if(get_option('gma_enable_email_delay')){
			add_action('gma_new_question_notify', array( $this, 'new_question_notify' ), 10, 2);
			add_action('gma_new_answer_notify', array( $this, 'new_answer_notify' ), 10, 2);
			add_action('gma_new_comment_notify', array( $this, 'new_comment_notify' ), 10, 2);

			add_action( 'gma_add_question', array( $this, 'gma_queue_add_question' ), 10, 2 );
			add_action( 'gma_add_answer', array( $this, 'gma_queue_add_answer' ), 10, 2 );
			add_action( 'wp_insert_comment', array( $this, 'gma_queue_insert_comment' ), 10, 2 );
		}else{
			add_action( 'gma_add_question', array( $this, 'new_question_notify' ), 10, 2 );
			add_action( 'wp_insert_comment', array( $this, 'new_comment_notify' ), 10, 2 );
			add_action( 'gma_add_answer', array( $this, 'new_answer_notify' ), 10, 2 );
		}

	}

	
	public function gma_queue_add_question($question_id, $user_id){
		wp_schedule_single_event( time() + 120, 'gma_new_question_notify', array($question_id, $user_id) );
	}
	public function gma_queue_add_answer($answer_id, $question_id){
		wp_schedule_single_event( time() + 120, 'gma_new_answer_notify', array($answer_id, $question_id) );
	}
	public function gma_queue_insert_comment($comment_id, $comment){
		wp_schedule_single_event( time() + 120, 'gma_new_comment_notify', array($comment_id, $comment) );
	}
	...
	public function new_question_notify( $question_id, $user_id ) {
		...
		$enabled = get_option( 'gma_subscrible_enable_new_question_notification', 1 );
		if ( ! $enabled ) {
			return false;
		}
		...
		$subject = get_option( 'gma_subscrible_new_question_email_subject' );
		if ( ! $subject ) {
			$subject = __( 'A new question was posted on {site_name}', 'give-me-answer-lite' );
		}
		...
		$message = gma_get_mail_template( 'gma_subscrible_new_question_email', 'new-question' );
		if ( ! $message ) {
			return false;
		}
		// Replacement
		...
		$message = str_replace( '{user_link}', gma_get_author_link( $user_id ), $message );
	...
	public function new_answer_notify( $answer_id, $question_id ) {
		...
		$site_url = site_url();
		$enable_send_copy = get_option( 'gma_subscrible_send_copy_to_admin' );
		$admin_email = $this->get_admin_email('answer');
		// for answer
		$answer_is_anonymous = gma_is_anonymous( $answer_id );
		if ( $answer_is_anonymous ) {
			$user_answer_id = 0;
			$user_answer_display_name = get_post_meta( $answer_id, '_gma_anonymous_name', true );
			$user_answer_display_name = $user_answer_display_name ? sanitize_text_field( $user_answer_display_name ) : __( 'Anonymous', 'give-me-answer-lite' );
			$user_answer_email = get_post_meta( $answer_id, '_gma_anonymous_email', true );
		...
		// for question
		$question_is_anonymous = gma_is_anonymous( $question_id );
		if ( $question_is_anonymous ) {
			$user_question_id = 0;
			$user_question_display_name = get_post_meta( $question_id, '_gma_anonymous_name', true );
			$user_question_display_name = $user_question_display_name ? sanitize_text_field( $user_question_display_name ) : __( 'Anonymous', 'give-me-answer-lite' );
			$user_question_email = get_post_meta( $question_id, '_gma_anonymous_email', true );
		...
		$answer_notify_enabled = get_option( 'gma_subscrible_enable_new_answer_followers_notification', 1 );
		if ( $answer_notify_enabled && !empty( $followers_email ) && is_array( $followers_email ) && 'private' !== get_post_status( $answer_id ) ) {
			$subject = get_option( 'gma_subscrible_new_answer_followers_email_subject', __( '[{site_name}] You have a new answer for your followed question', 'give-me-answer-lite' ) );
			...
			$message = gma_get_mail_template( 'gma_subscrible_new_answer_followers_email', 'new-answer-followers' );
			$message = apply_filters( 'gma_get_new_answer_email_to_followers_message', $message, $answer_id, $question_id );
		...
		$answer_notify_for_question_enabled = get_option( 'gma_subscrible_enable_new_answer_notification', 1 );
		if ( $user_question_email && $answer_notify_for_question_enabled && absint( $user_answer_id ) !== absint( $user_question_id ) ) {
			$subject = get_option( 'gma_subscrible_new_answer_email_subject', __( '[{site_name}] A new answer for "{question_title}" was posted on {site_name}', 'give-me-answer-lite' ) );
			$subject = str_replace( '{site_name}', esc_html( $site_name ), $subject );
			$subject = str_replace( '{question_title}', $question_title, $subject );
			...
			$message = gma_get_mail_template( 'gma_subscrible_new_answer_email', 'new-answer' );
			$message = apply_filters( 'gma_get_new_answer_email_to_author_message', $message, $question_id, $answer_id );
			if ( !$message ) {
				return false;
			}
	...
	public function new_comment_notify( $comment_id, $comment ) {
		...
		if ( 1 == $comment->comment_approved && ( 'gma-question' == $parent || 'gma-answer' == $parent ) ) {
			if ( $parent == 'gma-question' ) {
				$enabled = get_option( 'gma_subscrible_enable_new_comment_question_notification', 1 );
				// $admin_email = $this->get_admin_email( 'comment-question' );   //ignore in this time   
			} elseif ( $parent == 'gma-answer' ) {
				$enabled = get_option( 'gma_subscrible_enable_new_comment_answer_notification', 1 );
			...
			if ( gma_is_anonymous( $comment->comment_post_ID ) ) {
				$post_parent_email = get_post_meta( $comment->comment_post_ID, '_gma_anonymous_email', true );
				...
				if ( $post_parent->post_author != $comment->user_id ) {
					...
					if ( ! gma_is_followed( $post_parent->ID, $comment->user_id ) ) {
						add_post_meta( $post_parent->ID, '_gma_followers', $comment->user_id );
					}
				}
			...
			if ( $parent == 'gma-question' ) {
				$message = gma_get_mail_template( 'gma_subscrible_new_comment_question_email', 'new-comment-question' );
				$subject = get_option( 'gma_subscrible_new_comment_question_email_subject',__( '[{site_name}] You have a new comment for question {question_title}', 'give-me-answer-lite' ) );
				$message = str_replace( '{question_author}', get_the_author_meta( 'display_name', $post_parent->post_author ), $message );
				$question = $post_parent;
			} else {
				$message = gma_get_mail_template( 'gma_subscrible_new_comment_answer_email', 'new-comment-answer' );
				$subject = get_option( 'gma_subscrible_new_comment_answer_email_subject',__( '[{site_name}] You have a new comment for answer', 'give-me-answer-lite' ) );
				$message = str_replace( '{answer_author}', get_the_author_meta( 'display_name', $post_parent->post_author ), $message );
				$question_id = gma_get_post_parent_id( $post_parent->ID );
				$question = get_post( $question_id );
			}
			$subject = str_replace( '{site_name}', get_bloginfo( 'name' ), $subject );
			$subject = str_replace( '{question_title}', $question->post_title, $subject );
			...
			// logo replace
			$logo = get_option( 'gma_subscrible_email_logo','' );
			...
			$message = str_replace( '{comment_author_link}', gma_get_author_link( $comment->user_id ), $message );
			...
			$message = str_replace( '{site_url}', site_url(), $message );
			if ( $parent == 'gma-question' ) {
				$enable_notify = get_option( 'gma_subscrible_enable_new_comment_question_followers_notify', true );
			} else {
				$enable_notify = get_option( 'gma_subscrible_enable_new_comment_answer_followers_notification', true );
			}
			
			if ( $enable_notify ) {
				//Follower email task
				$followers = get_post_meta( $post_parent->ID, '_gma_followers' );
				$comment_email = get_the_author_meta( 'user_email', $comment->user_id );

				if ( $parent == 'gma-question' ) {
					$message_to_follower = gma_get_mail_template( 'gma_subscrible_new_comment_question_followers_email', 'new-comment-question' );
					$follow_subject = get_option( 'gma_subscrible_new_comment_question_followers_email_subject',__( '[{site_name}] You have a new comment for question {question_title}', 'give-me-answer-lite' )  );
					...
					$message_to_follower = gma_get_mail_template( 'gma_subscrible_new_comment_answer_followers_email', 'new-comment-answer' );
					$follow_subject = get_option( 'gma_subscrible_new_comment_answer_followers_email_subject',__( '[{site_name}] You have a new comment for answer', 'give-me-answer-lite' )  );
				...
				$message_to_follower = str_replace( '{comment_author_link}', gma_get_author_link( $comment->user_id ), $message_to_follower );
	...
	public function get_admin_email( $type = 'question' ){
		switch ($type) {
			case 'answer':
				$admin_email = get_option( 'gma_subscrible_new_answer_forward', '' );
				break;
			case 'comment-question':
				$admin_email = get_option( 'gma_subscrible_new_comment_question_forward', '' );
				break;
			case 'comment-answer':
				$admin_email = get_option( 'gma_subscrible_new_comment_answer_forward', '' );
				break;
			case 'question':
			default:
				$admin_email = get_option( 'gma_subscrible_sendto_address', '' );
				break;
		}
		$emails = preg_split('/\r\n|\r|\n/', $admin_email );
	...
	function new_activity( $post_id = 0 ) {
		...
		if ( 'gma_add_comment' == current_action() ) {
			$comment = get_comment( $post_id );
			$title = __( 'New Comment in: ', 'gma-notification' );
		...
		if ( 'gma-answer' == get_post_type( $post_id ) ) {
			...
			if ( !$title ) {
				$title = __( 'New Answer in: ', 'gma-notification' );
			}

		} else {
			$question_id = $post_id;
			if ( !$title ) {
				$title = __( 'New Question: ', 'gma-notification' );
			}
		}
	...
	public function get_from_address() {
		$from_email = get_option( 'gma_subscrible_from_address', get_bloginfo( 'admin_email' ) );
	...
	public function get_from_name() {
		$name = get_option( 'gma_subscrible_from_name', get_bloginfo( 'name' ) );
	...
	public function get_content_type() {
		return apply_filters( 'gma_notifications_get_content_type', 'text/html' );
	}
