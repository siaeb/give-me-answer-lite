...
class GMA_Posts_Comment {
	public function __construct() {
		...
		add_filter( 'gma_pre_comment_content', 'wp_kses_data', 10 );
	}
	/**
	...
	public function hook_redirect_comment_for_answer( $location, $comment ) {
		...
		if ( 'gma-answer' == get_post_type( $comment->comment_post_ID ) ) {
			$question_id = gma_get_post_parent_id( $comment->comment_post_ID );
			if ( $question_id ) {
	...
	/**
	 * Displays form token for unfiltered comments. Override wp_comment_form_unfiltered_html_nonce custom for gma
	 *
	...
	public function wp_comment_form_unfiltered_html_nonce() {
		...
		if ( current_user_can( 'unfiltered_html' ) 
				&& 'gma-answer' != get_post_type( $post_id )  ) {
	...
	public function comment_form_id_fields_filter( $result, $id, $replytoid ) {
		if ( 'gma-answer' == get_post_type( $id ) ) {
	...
	public function sanitizie_comment( $content, $comment ) {
		...
		if ( $post_type == 'gma-question' || $post_type == 'gma-answer' ) {
	...
	public function get_comments() {
		if ( isset( $_GET['post'] ) ) {
			...
			wp_list_comments( array( 
				'style' => 'ol',
				'callback'  => 'gma_question_comment_callback',
			), $comments ); 
		}
		exit( 0 );
	}
	...
	public function comment_author_link_anonymous( $comment ) {
		if ( $comment->user_id <= 0 && ( get_post_type( $comment->comment_post_ID ) == 'gma-question' || get_post_type( $comment->comment_post_ID ) == 'gma-answer' ) ) {
			$comment->comment_author = __( 'Anonymous','give-me-answer-lite' );
		}
		return $comment;
	}
	...
	public function reopen_question_have_new_comment( $comment_ID ){
		...
		$question_id = false;
		if ( 'gma-answer' == $comment_post_type ) {
			$question_id = gma_get_post_parent_id( $comment->comment_post_ID );
		} elseif ( 'gma-question' == $comment_post_type ) {
		...
		if ( $question_id ) {
			...
			if ( ! user_can( $comment->user_id, 'edit_posts' ) ) {
				if ( 'resolved' == $question_status ) {
					update_post_meta( $question_id, '_gma_status', 're-open' );
				}
			}
		}

		if ( $comment->user_id ) {
			add_post_meta( $question_id, '_gma_followers', $comment->user_id );
		}
	}
}
?>